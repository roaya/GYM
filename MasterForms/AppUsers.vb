Public Class AppUsers
    ...
    Dim BSourceApp_Users As New BindingSource
    '-------------------------------
    'Set The Data Table Name
    Dim TName As String = "App_Users"
    Dim d As Date
    '-------------------------------
    Private TblEmp As New GeneralDataSet.EmployeesDataTable
    Private TblSecH As New GeneralDataSet.Security_Group_HeaderDataTable
    ...
    Private Sub Customers_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            cls.RefreshData(TblSecH, "Security_Group_Header")
            ...
            Gcls = New GeneralSp.NewMasterForms(TName, BSourceApp_Users, BtnNew, BtnSave, BtnDelete, BtnSearch, BtnExit, BtnReload, BtnCancelSerach, BtnFirst, BtnPrevious, BtnNext, BtnLast, BtnFile, BtnData, BtnHelp, OrderByCombo, ContentPanel, MasterField1, CountRecords)

            '-------------------------------
            'Must Specify Window Title 
            '-------------------------------

            Gcls.SetWTitle = "»Ì«‰«  «·„” Œœ„Ì‰"
            Me.Text = Gcls.SetWTitle

            MasterField1.TextBox1.DataBindings.Add("Text", BSourceApp_Users, "User_Name")
            TxtPassword.DataBindings.Add("Text", BSourceApp_Users, "User_Password")

            EmployeeID.DataSource = TblEmp
            EmployeeID.DisplayMember = "Employee_Name"
            EmployeeID.ValueMember = "Employee_ID"
            EmployeeID.DataBindings.Add("SelectedValue", BSourceApp_Users, "Employee_ID")

            GroupID.DataSource = TblSecH
            GroupID.DisplayMember = "Group_Name"
            GroupID.ValueMember = "Group_ID"
            GroupID.DataBindings.Add("SelectedValue", BSourceApp_Users, "Group_ID")

            AccountStatus.DataBindings.Add("Checked", BSourceApp_Users, "Account_Status", True)
            SSource = BSourceApp_Users
    ...
    Private Sub OrderByCombo_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles OrderByCombo.SelectedIndexChanged
        Try
            If B_EndLoad = True Then
                Gcls.SortData(BSourceApp_Users, OrderByCombo.SelectedValue)
            End If
        Catch ex As Exception
            cls.WriteError(ex.Message, TName)
        End Try
